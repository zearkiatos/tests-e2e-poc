...
import { getRandomNumberTag } from '../../../mock/tag'

describe('Editar un Tag existente con campos únicamente numericos', () => {
  const sitePage = new SitePage()
  const signinPage = new SigninPage()
  const tagsPage = new TagsPage()
  const tagsEditorPage = new TagsEditorPage()

  const tag = getRandomNumberTag();

  beforeEach(function() {
  })
    let nameTag = tag.tag_name
    let colorTag = tag.tag_color
    ...
    it('Editar un Tag existente con campos únicamente numericos', () => {
        ...
        tagsEditorPage.ingresarNombre(nameTag)
